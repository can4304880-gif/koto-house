...
function initModals() {
  const performerModal = document.getElementById('performer-modal');
  const performerTitle = document.getElementById('performer-modal-title');
  const performerBody = document.getElementById('performer-modal-body');
  const contactModal = document.getElementById('contact-modal');
  const ticketModal = document.getElementById('ticket-modal');
  const guideContactModal = document.getElementById('guide-contact-modal');
  let activeTrigger = null;
  ...
  function getFormModal(trigger) {
    const formTarget = trigger.dataset.formModalOpen;
    if (formTarget === 'contact') return contactModal;
    if (formTarget === 'ticket') return ticketModal;
    if (formTarget === 'guide-contact') return guideContactModal;
    if (formTarget) {
      return document.getElementById(`${formTarget}-modal`) || document.getElementById(formTarget);
    }
  ...
  function handleOpenTrigger(trigger, event) {
    if (trigger.matches('[data-performer-modal-open]')) {
      event.preventDefault();
      if (performerTitle) performerTitle.textContent = '出演者プロフィール';
      if (performerBody) performerBody.textContent = profiles.join('\n\n');
      openModal(performerModal, trigger);
      return;
    }

    const modal = getFormModal(trigger);
    if (!modal) return;
    event.preventDefault();
    openModal(modal, trigger);
  }

  function bindTap(selector) {
    document.addEventListener('click', (event) => {
      ...
      handleOpenTrigger(trigger, event);
    });
  ...
  bindTap('[data-performer-modal-open], [data-form-modal-open], a[href="#ticket-modal"], a[href="#contact-modal"], a[href="#guide-contact-modal"]');

  document.querySelectorAll('[data-form-modal-close], [data-performer-modal-close]').forEach(button => {
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key !== 'Escape') return;
    const openModalEl = document.querySelector('.form-modal.is-open');
  ...
  document.querySelectorAll('[data-formspree-form]').forEach(form => {
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        form.classList.add('is-submitted');
        if (message) {
          message.textContent = modal && modal.id === 'ticket-modal'
            ? 'ありがとうございました。内容を確認のうえ、折り返しご連絡いたします。'
            : 'お問い合わせありがとうございました。';
          message.classList.add('is-visible');
        }
